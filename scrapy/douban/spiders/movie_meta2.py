...
class MovieMetaSpider(Spider):
    ...
    sql = 'SELECT * FROM subjects WHERE type="movie" AND douban_id NOT IN (SELECT douban_id FROM movies) ORDER BY douban_id ASC'
...
lpst').replace('mpic', 'lpic')
    ...
    def get_director_ids(self, meta, response):
        ...
        meta['director_ids'] = validator.process_slash_str('/'.join(director_ids))
        return meta
